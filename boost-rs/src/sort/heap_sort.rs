...
pub fn heap_sort<T: PartialOrd>(arr: &mut [T]) {
    heap_sort_with_comparator(arr, |x: &T, y: &T| x.lt(y))
}

/// Sort a mutable slice using heap sort and custom comparator.
    ...
    F: Fn(&T, &T) -> bool,
{
    ...
    for end in (1..arr.len()).rev() {
        arr.swap(0, end);
        move_down(&mut arr[..end], 0, &is_less);
    }
}

/// Convert `arr` into a max heap.
fn heapify<T, F>(arr: &mut [T], is_less: &F)
where
    F: Fn(&T, &T) -> bool,
{
    let last_parent = (arr.len() - 2) / 2;
    for i in (0..=last_parent).rev() {
        move_down(arr, i, is_less);
    }
}
...
fn move_down<T, F>(arr: &mut [T], mut root: usize, is_less: &F)
where
    F: Fn(&T, &T) -> bool,
{
    let last = arr.len() - 1;
    loop {
        ...
        let right = left + 1;
        let max = if right <= last && is_less(&arr[left], &arr[right]) {
            right
        } else {
            left
        };

        if is_less(&arr[root], &arr[max]) {
            arr.swap(root, max);
        }
        root = max;
    }
